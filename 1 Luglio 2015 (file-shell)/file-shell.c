...
typedef struct{
    ...
    char text[TEXT_DIM];    // una stringa o
        ...
        if(token=strtok(buffer, " ")){  // estrae il primo token: il comando
            if(strcmp(token,"list")==0){
                if(token=strtok(NULL, " ")){ // estrae il secondo token: il numero del figlio 
                    i=atoi(token);
                    if((i>=1) && (i<=children)){
                        /* invia un messaggio al figlio indicato con la richiesta CMD_LIST */
                        message.type=i;
                ...
                if(token=strtok(NULL," ")){ // estrae il secondo token: il numero del figlio
                    ...
                    if((i>=1) && (i<=children)){
                        ...
                        if(token=strtok(NULL," ")){     // estrae il terzo token: il nome del file 
                            /* invia un messaggio al figlio indicato con la richiesta CMD_SIZE */
                ...
                if(token=strtok(NULL," ")){     // estrae il secondo token: il mumero del figlio 
                    i=atoi(token);
                    if((i>=1) && (i<=children)){
                        message.type=i;
                        if(token=strtok(NULL," ")){     // estrae il terzo token: il nome del file
                            ...
                            if(token=strtok(NULL," ")){     // estrae il quarto token: la parola da cercare
                                /* invia il primo messaggio solo dopo essere iscuri di avere la parola */
                                msgsnd(queue,&message,sizeof(msg)-sizeof(long),0);
