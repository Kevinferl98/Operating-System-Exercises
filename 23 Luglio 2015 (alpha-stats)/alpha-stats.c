...
void child(int num, char *file, int queue){
    ...
    /* ricava informazioni sul file e controlla se è un file regolare */
    ...
    if(((fd=open(file,O_RDONLY))==-1) || ((map=(char *)mmap(NULL,statbuf.st_size,PROT_READ,MAP_SHARED,fd,0))==MAP_FAILED)){
        message.type=2;
        msgsnd(queue,&message,sizeof(message)-sizeof(long),0);
        exit(2);
    }
    /* calcola le occorenze delle sole lettere */
